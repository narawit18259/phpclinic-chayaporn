...
$Thick_Thui_Massage = $_POST['Thick_Thui_Massage'];
$Guasa = $_POST['Guasa'];
$chinese_medicine = $_POST['chinese_medicine'];
$date_appointment = $_POST['date_appointment'];
$id = $_POST['id'];
mysqli_query($conn, "INSERT INTO treatment (weight, height, perssure, symptoms, Af, 
                            Beauty_acupuncture, Acupuncture_weight_loss, Cover_glass, Thick_Thui_Massage, Guasa, chinese_medicine, date_appointment, date_data, id)
                            VALUES ('$weight', '$height', '$perssure','$symptoms','$Af','$Beauty_acupuncture'
                            ,'$Acupuncture_weight_loss','$Cover_glass','$Thick_Thui_Massage','$Guasa','$chinese_medicine','$date_appointment',NOW(),'$id')");
$sql = "SELECT * FROM treatment inner join newcase on treatment.id = newcase.id where treatment.id = '$id' order by treatment_id desc  " or die("Error:" . mysqli_error());
    ...
    <div class="fonttext">
        ...
        <form method="post" action="exroomsave.php">
            ...
            <div class="container-fluid">
                <div class="jumbotron">
                    <h2 style="text-align: center;" class="h">เพิ่มข้อมูลเสร็จสิ้น</h2>
                    <table class="table table-striped">
                        <?php
                        //หัวข้อตาราง
                        echo "<tr align='center' bgcolor='#CCCCCC'><td>ชื่อ</td><td>นามสกุล</td><td>น้ำหนัก</td><td>ส่วนสูง</td><td>ความดัน</td><td>อาการป่วย</td><td>วันนัดครั้งถัดไป</td><td>แก้ไข</td><td>ลบ</td></td></td></tr>";
                        ...
                        echo "<td align='center'><a href='editexroom.php?ID=" . $row["id"] . "' class='btn btn-outline-warning'>แก้ไข</a></td> ";
                        echo "<td align='center'><a href='deletexroom.php?ID=" . $row["id"] . "' class='btn btn-outline-danger' onclick=\"return confirm('คุณต้องการที่จะลบข้อมูลนี้หรือไม่ ? !!!' )\">ลบ</a></td> ";
                        echo "</tr>";
                        mysqli_close($conn);
                        ?>
                    </table>
